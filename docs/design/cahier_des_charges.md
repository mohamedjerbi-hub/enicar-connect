...
### 5.1 Réservation de Salles et Laboratoires
Calendriers de disponibilité, demande en ligne, validation par l'administration.
### 5.2 Gestion des Absences (QR Code)
Génération de QR codes pour pointage immédiat en TP/Cours, alertes d'absentéisme.
...
Attestations diverses (scolarité, réussite, stage), génération automatique, signature électronique.
...
### 6.1 Architecture
- **Frontend :** PWA, Responsive (Angular 15+).
- **Backend :** API RESTful via Microservices (Spring Boot 3).
- **Bases de données :** SGBD Relationnel (PostgreSQL) + NoSQL pour données dynamiques.
